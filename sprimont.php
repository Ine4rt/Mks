        ...
        if (xhr.status === 200) {
            try {
                ...
                if (order.success) {
                    ...
                    let init = order.order.initials || 'NON';
...
let titrePack = (role.toLowerCase() === 'keeper') ? 'Pack Sprimont Keeper' : 'Pack Sprimont';

if (role.toLowerCase() === "keeper") {
    var titles = {
        veste: "Veste",
        pantalon: "Pantalon",
        kit: "Kit",
        bas: "Bas Keeper",
        souspull: "Sous-Pull Keeper",
        kway: "Option Kway"
    };
	var references = {
        veste: "200527504",
    ...
    font-weight: normal;
                        ...
                        <body>
                            <div class="container">
                                ...
                                <img src="sprimont.png" class="logo" alt="Logo Club">
                                <h2>${titrePack}</h2>
                                ...
                                <div class="grid-container">
                                    <div class="item-box">
    <div class="item-title">${titles.veste}
	<div class="item-reference">${references.veste}</div> 
	</div>
    <!-- Nouveau bloc pour la référence -->
    <div class="item-size">${veste}</div>
</div>

                                    <div class="item-box">
                                        <div class="item-title">${titles.pantalon}
										<div class="item-reference">${references.pantalon}</div> <!-- Nouveau bloc pour la référence --></div>
										
                                        <div class="item-size">${pantalon}</div>
                                    </div>
                                    <div class="item-box">
                                        <div class="item-title">${titles.kit}
										<div class="item-reference">${references.kit}</div> <!-- Nouveau bloc pour la référence --></div>
										
                                        <div class="item-size">${kit}</div>
                                    </div>
                                    <div class="item-box">
                                        <div class="item-title">${titles.bas}
										<div class="item-reference">${references.bas}</div> <!-- Nouveau bloc pour la référence --></div>
										
                                        <div class="item-size">${bas}</div>
                                    </div>
                                    <div class="item-box">
                                        <div class="item-title">${titles.souspull}
										<div class="item-reference">${references.souspull}</div> <!-- Nouveau bloc pour la référence --></div>
										
                                        <div class="item-size">${souspull}</div>
                                    </div>
                                </div>
                                ...
                                <div class="grid-container">
                                    <div class="item-box">
                                        <div class="item-title">${titles.kway}
										<div class="item-reference">${references.kway}</div> <!-- Nouveau bloc pour la référence --></div>
										
                                        <div class="item-size">${kway}</div>
                                    </div>
                                    <div class="item-box">
...
<div id="filterContainer">
    ...
    <button class="filter-btn" onclick="filterByRole('U6-U9', this)">U6-U9</button>
    <button class="filter-btn" onclick="filterByRole('U10-U21', this)">U10-U21</button>
	<button class="filter-btn" onclick="filterByRole('Entraineur', this)">Entraineur</button>
</div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th>Veste </br> 100225643</th>
                <th>Pantalon </br> 100522102</th>
                <th>Maillot </br> 100225340</th>
               
				<th>2 Bas </br> 100330220</th>
				 <th>Short </br> 100530641</th>
                
                <th>Polo </br> 100225943</th>
                <th>Initiales</th>
...
echo "<td>" . $date->format('d/m') . "</td>";
                ...
                echo "<td data-column='email'>" . htmlspecialchars($row['email']) . "</td>";
                ...
                echo "'>";
                if ($row['role'] == 'U6-U9') {
                    echo "👶🏻"; // Icône ballon de foot
                } elseif ($row['role'] == 'U10-U21') {
                    echo "⚽️"; // Icône paire de gants
                }
                echo "</td>";
                ...
                echo "<td onclick=\"toggleMissing(" . $row['id'] . ", 'jersey_missing', this)\" class='" . ($row['jersey_missing'] ? 'missing' : '') . "'>" . htmlspecialchars($row['jersey_size']) . "</td>";
                echo "<td onclick=\"toggleMissing(" . $row['id'] . ", 'bas_missing', this)\" class='" . ($row['bas_missing'] ? 'missing' : '') . "'>" . htmlspecialchars($row['bas_size']) . "</td>";
                echo "<td onclick=\"toggleMissing(" . $row['id'] . ", 'short_missing', this)\" class='" . ($row['short_missing'] ? 'missing' : '') . "'>" . htmlspecialchars($row['short_size']) . "</td>";
echo "<td onclick=\"toggleMissing(" . $row['id'] . ", 'polo_missing', this)\" class='" . ($row['polo_missing'] ? 'missing' : '') . "'>" . htmlspecialchars($row['polo_size']) . "</td>";
...
function toggleFilter() {
    ...
    rows.forEach(row => {
        ...
        if (roleCell) {
            ...
            if (currentFilter === "all" || role === "👶🏻" && currentFilter === "U6-U9" || role === "⚽️" && currentFilter === "U10-U21") {
                row.style.display = "";
            } else {
...
function filterByRole(role, button) {
    ...
    rows.forEach(row => {
        ...
        if (roleCell) {
            ...
            if (role === "all" || (role === "U6-U9" && roleText === "👶🏻") || (role === "U10-U21" && roleText === "⚽️")) {
                row.style.display = "";
            } else {
                row.style.display = "none";
            }
